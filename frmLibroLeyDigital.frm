...
Begin VB.Form frmLibroLeyDigital 
   ...
   Begin DDSharpGridOLEDB2.SGGrid sgEmpresas 
      ...
      ValueItems      =   $"frmLibroLeyDigital.frx":6699
   End
...
On Error GoTo Salir
    ...
    Dim cnEmpresa As ADODB.Connection
    ...
    Dim sAuxNombreArchivo As String 'Acá armamos el nombre del archivo de texto para el libro  ley
    ...
    Set cnEmpresa = New ADODB.Connection
...
'    End If
    ...
    cnEmpresa.Open SGINST
    rs.CursorLocation = adUseClient
    
    rs.Open "SELECT ISNULL(EMPRCUIT,0) AS EMPRCUIT FROM SGEMPRESA WHERE EMPR_ID = " & Me.sgEmpresas.Rows.Current.Cells(3).value, cnEmpresa, adOpenDynamic, adLockReadOnly
    
    If rs!EMPRCUIT = 0 Then
        MsgBox "Falta definir el parametro de CUIT de la empresa. No se puede continuar.", vbCritical, "Información"
        Exit Sub
        rs.Close
        cnEmpresa.Close
    ...
    If cnEmpresa.State = 1 Then cnEmpresa.Close
...
'    Open sCarpeta & "\LibroLey.txt" For Output As Arch
    ...
    sAuxNombreArchivo = "LibroLey_" & Me.sgEmpresas.Rows.Current.Cells(7).value & "_L" & Trim(Me.txtNumero.Text) & ".txt"
    
    Open sCarpeta & "\" & sAuxNombreArchivo For Output As Arch
'que sea libroley E nro empresa y L nro liquidacion, por ejemplo, para liquidación 2 y empresa 3, LibroLey_E03_L02.txt
    ...
    rs4.Open "Select * from LibroLey_Reg04 order by Cuil", cn, adOpenDynamic, adLockReadOnly
...
Salir:
    MsgBox "Se ha producido un error. Comuníquese con mantenimiento.", vbCritical, "Error"
    MsgBox "Error: " & Err.Number & vbCrLf & "Descripción: " & Err.Description, vbCritical, "Error"
    MsgBox "Legajo: " & sLegajo, vbCritical, "Error"
...
Private Sub LlenarEmpresas()
    ...
    cn.Open SGINST
    rs.Open "Select *,REPLACE(EMPRCODIGO,'E','E0') AS EMPR_CODIGO from sgEmpresa where EmprOrden > 100 order by EmprOrden", cn, adOpenDynamic, adLockBatchOptimistic, adCmdText
